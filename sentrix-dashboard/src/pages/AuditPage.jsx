...
export function AuditPage({ user }) {
  ...
  useEffect(() => {
    if (pendingRevoke) {
      setUnblockTarget({
        ip:
          pendingRevoke.block_target === "all" ||
          pendingRevoke.block_target === "ip",
        mac:
          pendingRevoke.block_target === "all" ||
          pendingRevoke.block_target === "mac",
      });
    }
  }, [pendingRevoke]);

  const { currentPage, pageSize, setCurrentPage, setPageSize } =
    usePaginationState("audit", 5);

  const filteredLogs = useMemo(
    () =>
      logs.filter((log) =>
        matchesSearch(log, query, [
          "actorEmail",
          "actorRole",
          "action",
          "targetLabel",
  ...
  async function confirmRevoke() {
    ...
    try {
      const target =
        unblockTarget.ip && unblockTarget.mac
          ? "all"
          : unblockTarget.ip
            ? "ip"
            : "mac";
  ...
  useEffect(() => {
    ...
    socket.on("audit:new", (newLog) => {
      ...
      setLogs((prev) => {
        ...
        if (
          startDate &&
          newLog.createdAt < new Date(`${startDate}T00:00:00.000`).getTime()
        )
          return prev;
        if (
          endDate &&
          newLog.createdAt > new Date(`${endDate}T23:59:59.999`).getTime()
        )
          return prev;
    ...
    socket.on("authority:update", ({ category }) => {
      ...
      if (activeTab === "whitelist" && category === "whitelist")
        loadAuthority();
      if (activeTab === "ratelimit" && category === "rate_limit")
        loadAuthority();
      if (activeTab === "perimeter" && category === "blacklist")
        loadAuthority();
    });
  ...
  return (
    <div className="page-reveal space-y-6">
      {/* Whitelist Modal */}
      <BlurOverlay
        isOpen={isAddingWhitelist}
        onClose={() => setIsAddingWhitelist(false)}
        className="z-[80]"
        containerClassName="w-full max-w-lg"
      >
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start gap-4">
            <span className="grid h-12 w-12 shrink-0 place-items-center rounded-lg border border-emerald-100 bg-emerald-50 text-emerald-600">
              <Plus size={24} />
            </span>
            ...
            <button
              ...
              onClick={() => setIsAddingWhitelist(false)}
              type="button"
            >
              <X size={18} />
            </button>
          ...
          <div className="mt-6 space-y-4">
            <div>
              ...
              <input
                className="w-full h-11 px-4 rounded-lg border border-slate-200 text-sm font-medium outline-none focus:border-slate-900"
                placeholder="e.g. Admin Phone, Lab Printer"
                value={newWhitelist.label}
                onChange={(e) =>
                  setNewWhitelist({ ...newWhitelist, label: e.target.value })
                }
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  className="w-full h-11 px-4 rounded-lg border border-slate-200 text-sm font-medium outline-none focus:border-slate-900"
                  placeholder={
                    newWhitelist.type === "ip" ? "192.168.1.1" : "00:11:22..."
                  }
      ...
      >
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <textarea
            className="mt-5 min-h-24 w-full rounded-lg border border-slate-200 p-3 text-sm font-medium outline-none focus:border-slate-900"
          ...
          <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            >
              <div
                className={`flex items-center justify-center gap-2 ${processing ? "opacity-0" : "opacity-100"}`}
              >
      ...
      >
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start gap-4">
            <span
              className={`grid h-12 w-12 shrink-0 place-items-center rounded-lg border ${activeTab === "whitelist" ? "border-rose-100 bg-rose-50 text-rose-600" : activeTab === "perimeter" ? "border-rose-100 bg-rose-50 text-rose-600" : "border-emerald-100 bg-emerald-50 text-emerald-600"}`}
            >
              ...
              <div className="mt-6 p-4 rounded-lg border border-slate-100 bg-slate-50/50 space-y-3">
                ...
                <div className="flex flex-wrap gap-4">
                  {pendingRevoke?.ip_address && (
                    <label
                      className={`flex items-center gap-2 cursor-pointer group ${pendingRevoke.block_target === "mac" ? "opacity-50 grayscale cursor-not-allowed" : ""}`}
                    >
                      <input
                        type="checkbox"
                        ...
                        disabled={
                          processing || pendingRevoke.block_target === "mac"
                        }
                      />
                      <span
                  ...
                  {pendingRevoke?.mac_address && (
                    <label
                      className={`flex items-center gap-2 cursor-pointer group ${pendingRevoke.block_target === "ip" ? "opacity-50 grayscale cursor-not-allowed" : ""}`}
                    >
                      <input
                        type="checkbox"
                        className="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900"
                        ...
                        disabled={
                          processing || pendingRevoke.block_target === "ip"
                        }
                      />
                      <span
                ...
                <p className="text-[10px] italic text-slate-400">
                  ...
                  unblockTarget.ip &&
                  unblockTarget.mac
                    ? "Full restoration: Device can reconnect immediately."
                    : (unblockTarget.ip ||
                          pendingRevoke.block_target === "mac") &&
                        (unblockTarget.mac ||
                          pendingRevoke.block_target === "ip")
                      ? "Full restoration: Restoring remaining identifier."
                      : unblockTarget.ip
                        ? "Partial: IP will be restored, hardware remains blocked."
                        : unblockTarget.mac
                          ? "Partial: Hardware will be restored, IP remains blocked."
          ...
          <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            >
              ...
              {processing && (
                <div
                  className={`absolute inset-0 flex items-center justify-center backdrop-blur-[1px] ${activeTab === "whitelist" || activeTab === "perimeter" ? "bg-rose-600/50" : "bg-emerald-600/50"}`}
                >
                  <LoaderCircle className="animate-spin" size={18} />
                </div>
              )}
      ...
      <SearchFilterBar
        count={
          activeTab === "logs" ? filteredLogs.length : filteredAuthority.length
        }
        onQueryChange={setQuery}
        placeholder={
          activeTab === "logs"
            ? "Search by user, action, target, IP, or MAC"
            : "Search by label, identifier, or reason"
        }
        query={query}
      />
